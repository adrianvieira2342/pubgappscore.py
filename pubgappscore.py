...
if not df_bruto.empty:
    ...
    def renderizar_ranking(df_local, col_score, formula, explicacao, calculo_discreto=""):
        ...
        st.markdown(
            f"<div style='background-color:#161b22;padding:12px;border-radius:8px;border-left:5px solid #0078ff;margin-bottom:10px;text-align:left;'>💡 {explicacao}</div>",
            unsafe_allow_html=True
        )
        if calculo_discreto:
        ...
        # Formatação
        if col_score == "score":
            format_dict = {col: (lambda x: f"- {abs(x):.2f}" if isinstance(x, float) else f"- {int(abs(x))}") for col in cols_calc + ["kr", "kill_dist_max"]}
            format_dict[col_score] = "{:.2f}"
        else:
            format_dict = {col: "{:d}" for col in cols_calc}
        ...
        st.dataframe(
            ...
            hide_index=True,
            column_config={"nick": "Nickname", "kr": "K/R", "kill_dist_max": "Kill Dist Máx", "dano_medio": "Dano Médio"}
        )
    ...
    with tab1:
        ...
        renderizar_ranking(df_valid.copy(), "Score_Pro", f_pro, "Fórmula PRO: Valoriza equilíbrio entre sobrevivência e agressividade.", "(KR × 40) + (Dano Médio / 8) + (Win Rate × 500)")
    ...
    with tab2:
        ...
        renderizar_ranking(df_valid.copy(), "Score_Team", f_team, "Fórmula TEAM: Foco total no jogo coletivo.", "(Win Rate × 1000) + (Média Revives × 50) + (Média Assists × 35)")

    with tab3:
        f_elite = (df_valid["kr"] * 50) + ((df_valid["headshots"] / df_valid["partidas_calc"]) * 60) + (df_valid["dano_medio"] / 5)
        renderizar_ranking(df_valid.copy(), "Score_Elite", f_elite, "Fórmula ELITE: Prioriza K/R, precisão de Headshots e volume de dano.", "(KR × 50) + (Média Headshots × 60) + (Dano Médio / 5)")

    with tab4:
        if not df_bots_raw.empty:
            df_bots = df_bots_raw[df_bots_raw["partidas"] > 0].copy()
            if not df_bots.empty:
                renderizar_ranking(df_bots, "score", None, "Anti-Casual: Jogadores penalizados por matar bots.")
            else:
